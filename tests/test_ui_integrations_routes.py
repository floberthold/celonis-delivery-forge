...
os.environ["FORGE_DATABASE_URL"] = "sqlite:///./tmp_ui_integrations_routes_test.db"
...
DB_FILE = Path("tmp_ui_integrations_routes_test.db")
...
def _seed_user() -> tuple[str, str]:
    with Session(db_module.engine) as session:
        person = Person(
            email="ui-integrations@example.com",
            name="UI Integrations Tester",
        ...
        organization = Organization(name="UI Integrations Org", slug="ui-integrations-org")
...
def test_methodology_ui_route_renders() -> None:
    _reset_db()
