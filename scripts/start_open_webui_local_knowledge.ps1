...
if (Get-Command open-webui -ErrorAction SilentlyContinue) {
    Write-Host "Starting Open WebUI from PATH on http://$Host:$Port"
...
if (Test-Path $candidateExe) {
    Write-Host "Starting Open WebUI from local-wiki-query virtual environment on http://$Host:$Port"
    & $candidateExe serve --host $Host --port $Port
    exit $LASTEXITCODE
}
